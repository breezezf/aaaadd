...
#     for i in range(5):
#         st.write(' ')
#     st.link_button('离线版钢铁雄心四下载网址', 'https://pan.quark.cn/s/d3c686e3cfa0')
#     for i in range(10):
#         st.write(' ')
...
def page_6():
    ...
    if an:
        ...
        if st.button('处理'):
            ...
            left = left_sound[number1*1000:number2*1000]
            
            right = AudioSegment.empty()
            
            right = right_sound[number3*1000:number4*1000]
            
            new_sound = AudioSegment.from_mono_audiosegments(left,right)
            
            
            new_sound.export("新音乐.mp3",format="mp3")
            with open('新音乐.mp3', 'rb') as f:
                mymp3 = f.read()
            st.audio(mymp3, format='audio/mp3', start_time=0)
            
            
            st.download_button(label = "新音乐下载",data = mymp3,file_name = "新音乐.mp3")
            # if st.button("播放"):
            #     st.(new_sound)
